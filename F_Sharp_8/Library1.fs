...
let printPageStatAsync url =
    ...
    async {
        ...
        let printStatFor (url, html : string) = 
            printfn "%s --- %i" url html.Length
        let! html, _ = getHtml url
        printStatFor (html, url)
        parseHtml html |> Seq.map getHtml |> Async.Parallel |> Async.RunSynchronously |> Seq.map printStatFor |> ignore
    }